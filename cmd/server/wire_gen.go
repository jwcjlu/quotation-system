...
func wireApp(bootstrap *conf.Bootstrap, bootstrapProxy *conf.BootstrapProxy, logger log.Logger) (*kratos.App, func(), error) {
	...
	bomManufacturerCleaningRepo := data.NewBomManufacturerCleaningRepo(dataData, cachedBomManufacturerAliasRepo)
	...
	bomLineGapRepo := data.NewBOMLineGapRepo(dataData)
	bomMatchRunRepo := data.NewBOMMatchRunRepo(dataData)
	authService := service.NewAuthService(userAccountRepo, userSessionRepo)
	...
	bomService := service.NewBomService(bomSessionRepo, bomSearchTaskRepo, bomLineGapRepo, bomMatchRunRepo, bomMergeDispatch, openAIChat, bomFxRateRepo, cachedBomManufacturerAliasRepo, hsModelMappingRepo, hsItemReadRepo, hsTaxRateDailyRepo, taxRateAPIBizAdapter, bootstrap, logger)
	bomService.SetManufacturerCleaningRepo(bomManufacturerCleaningRepo)
	...
	httpServer := server.NewHTTPServer(bootstrap, logger, agentService, scriptPackageAdmin, authService, bomService, agentAdminService, hsResolveService, hsMetaService, hsSyncService)
