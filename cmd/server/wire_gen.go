...
func wireApp(bootstrap *conf.Bootstrap, bootstrapProxy *conf.BootstrapProxy, logger log.Logger) (*kratos.App, func(), error) {
	...
	hsPolicyRepo := data.NewHSPolicyRepo(dataData)
	hsCaseRepo := data.NewHSCaseRepo(dataData)
	hsReviewRepo := data.NewHSReviewRepo(dataData)
	hsFinalDecisionEngine := biz.NewHSFinalDecisionEngine()
	hsClassifyUsecase := biz.NewHSClassifyUsecase(hsPolicyRepo, hsCaseRepo, hsReviewRepo, hsFinalDecisionEngine, logger)
	...
	bomService := service.NewBomService(bomSessionRepo, bomSearchTaskRepo, bomMergeDispatch, hsClassifyUsecase, openAIChat, bomFxRateRepo, cachedBomManufacturerAliasRepo, bootstrap, logger)
