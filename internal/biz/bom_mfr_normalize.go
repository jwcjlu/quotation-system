...
type AliasLookup interface {
	CanonicalID(ctx context.Context, aliasNorm string) (canonicalID string, ok bool)
}

// ResolveManufacturerCanonical 将 BOM/报价原始厂牌串解析为规范 ID（严格模式）。
// 若 raw 在去空白后为空：返回 ("", false)，表示 §2.5 不施加厂牌约束（与别名未命中同为 hit=false，调用方需结合 TrimSpace(raw) 区分）。
// 若非空：经 NormalizeMfrString 后查表；未命中则 ("", false)（§2.3 严格策略）。
func ResolveManufacturerCanonical(ctx context.Context, raw string, lookup AliasLookup) (canonicalID string, hit bool) {
	...
	id, ok := lookup.CanonicalID(ctx, normKey)
	if !ok {
		return "", false
	}
	return id, true
