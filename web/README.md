...
- **BOM 会话**：`POST /api/v1/bom-sessions` → `POST /api/v1/bom/upload`（body 带 `session_id`）→ **会话看板**维护行、勾选平台 `PUT .../platforms`（与 [接口清单](../docs/BOM货源搜索-接口清单.md) 流程对齐；清单中 multipart 专用上传路径以后端实现为准）
